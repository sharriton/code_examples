...
{
    std::cout << "\n<Are PRO csv's must be downloaded and in the same folder as the BPB_New_Library .exe>";
    std::cout << "\nWould you like to navigate to the PRO websites? y/n>\n";
    getc(stdin);           // to flush the new line stuck in buffer
    char c = getc(stdin);
    if (c == 'n')
        return false;
    else
...
//    }
    ...
    {
        get_one_field(a);                                      // ignoring source id
...
//            std::cout << "\n" << temp;
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            temp_share.emplace_back(get_one_field(a));
