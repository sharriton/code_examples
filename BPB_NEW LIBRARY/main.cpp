...
int main()
{
    ...
    if(std::thread::hardware_concurrency() < 3)                             // check if there are available threads
    { std::cout << "\n Concurrency unavailable"; run_concurrent = false; }
    ...
    {
        ascap_import_data_and_search_titles(bpb_data, "./ascap.csv");
        sesac_import_data_and_search_titles(bpb_data, "./sesac.csv");
        bmi_import_data_and_search_titles(bpb_data);
    }
